...
// import { DayOfWeekSelector } from "./DayOfWeekSelector"; // Reusing your existing component
...
const AppLockSheet = forwardRef<AppLockSheetRef, AppLockSheetProps>(
  ({ onSave, onSelectApps, onSelectDuration, onSelectDifficulty }, ref) => {
    ...
    const [activeDays, setActiveDays] = useState([1, 2, 3, 4, 5]); // Mon-Fri default

    const snapPoints = useMemo(() => ["85%"], []);

    const handleIncrement = () => setUnlocksAllowed(prev => prev + 1);
    const handleDecrement = () => setUnlocksAllowed(prev => (prev > 1 ? prev - 1 : 1));
    ...
    return (
      ...
      >
        <BottomSheetView className="flex-1 px-5 pt-2">
          ...
          >
            <Text className="text-white text-base font-medium">Apps Locked</Text>
          ...
          <View className="bg-zinc-800 rounded-xl p-4 gap-6 mb-4">
            ...
            <View className="flex-row justify-between items-center">
                <View>
                    <Text className="text-white text-base font-medium">Unlocks Allowed</Text>
                ...
                <View className="flex-row items-center gap-3">
                    ...
                    <Text className="text-white text-lg font-bold w-6 text-center">{unlocksAllowed}</Text>
                    <Pressable 
            ...
            {/* Duration Selector */}
            <Pressable onPress={onSelectDuration} className="flex-row justify-between items-center">
                <Text className="text-white text-base font-medium">For Up To</Text>
                <View className="flex-row items-center">
            ...
            {/* Difficulty Selector */}
            <Pressable onPress={onSelectDifficulty} className="flex-row justify-between items-center">
                <Text className="text-white text-base font-medium">Difficulty</Text>
                <View className="flex-row items-center">
          ...
          <View className="bg-zinc-800 rounded-xl p-4 mb-6">
            <Text className="text-zinc-400 text-xs font-bold mb-4 uppercase">Days of week active</Text>
            <DayOfWeekSelector activeDays={activeDays} onToggleDay={handleToggleDay} />
          </View>
          ...
          <View className="mt-auto pb-6">
            <Pressable 
                ...
                style={{ backgroundColor: "#bbf7d0" }} // Matches the light green in image
            >
