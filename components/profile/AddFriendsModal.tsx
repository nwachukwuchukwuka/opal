...
const FOUND_FRIENDS = [
  { id: '1', name: 'Taaffeite5892', username: 'Taaffeite5892', avatarColor: '#10b981', isFriend: true },
];

const CONTACTS = [
  { id: '2', name: 'Jack Doe', avatarUrl: 'https://randomuser.me/api/portraits/men/32.jpg' },
  { id: '3', name: 'Jessica Smith', avatarUrl: 'https://randomuser.me/api/portraits/women/44.jpg' },
  { id: '4', name: 'John Smith', avatarUrl: 'https://randomuser.me/api/portraits/men/12.jpg' },
];
        ...
        <ScrollView className="flex-1 px-4" showsVerticalScrollIndicator={false}>
          <View className="gap-3 pb-24">
            ...
            {FOUND_FRIENDS.map((friend) => (
              <View key={friend.id} className="bg-zinc-900 p-4 rounded-2xl flex-row items-center justify-between">
                <View className="flex-row items-center gap-3">
                  <HexagonAvatar color={friend.avatarColor} size={40} />
                  <View>
                    <Text className="text-white font-semibold text-base">{friend.name}</Text>
                    <Text className="text-zinc-500 text-xs">{friend.username}</Text>
                  </View>
                </View>
                <View className="bg-zinc-800/50 px-4 py-2 rounded-full border border-zinc-700/50 flex-row items-center gap-1">
                  ...
                  <Text className="text-sky-400 font-bold text-sm">Friends</Text>
                </View>
              </View>
            ))}
            ...
            {filteredContacts.map((contact) => (
              <View key={contact.id} className="bg-zinc-900 p-4 rounded-2xl flex-row items-center justify-between">
                <View className="flex-row items-center gap-3">
                  <Image source={{ uri: contact.avatarUrl }} className="w-10 h-10 rounded-full" />
                  <View>
                    <Text className="text-white font-semibold text-base">{contact.name}</Text>
                    <Text className="text-zinc-500 text-xs uppercase tracking-wider">IN YOUR CONTACTS</Text>
                  </View>
                </View>
